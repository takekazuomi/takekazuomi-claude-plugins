...
└── scripts/
    ...
    └── lint.sh         # AI由来の語句・構文の型の検出（実行のみ、読み込まない）
```
...
| ファイル | 責務 | ロードされる条件 |
| --- | --- | --- |
| `SKILL.md` | ルーティング（種別判定）＋書き始める前の確認＋共有レビュー手順（9工程、チェックリストを関門にしたループ）＋frontmatter（name/description） | description は常時、本体はスキル起動時 |
| `styles/common.md` | 書き始める前に決めること（読み手・アウトプット・課題・主張）・引用の根拠URL・根拠と断定の較正・mannered prose の除去・技術用語/コマンド例/ファイル参照の記法 | 起動後、両スタイルとも必ず読む |
| `styles/casual.md` | 著者の感情・評価・判断・体験の扱い・3部構成・脚注設計・段落と文末の散文ルール | casual 判定時のみ |
...
| `scripts/lint.sh` | AI由来の語句・構文の型の検出（ERROR・WARN・TODO件数・箇条書き比率） | レビュー手順で必ず実行（内容は読まない） |
...
### AI臭対策の規定を common に置いた理由（2026-09-19）

AI臭い文章の問題と対策は [AI臭い文章の問題と対策](../ai-prose.md) にまとめ、その8節を外側のハーネス（不変条件 I1〜I9）とした。`common.md` に次を追加した。

- **根拠と断定の強さを揃える**：事実の記述で、材料の無い箇所を修辞や断定で埋めず、`TODO(未確認: ...)` として残す。穴を埋めるかどうかは文体ではなく根拠の問題なので、両スタイルに同じ形で効く（I1・I4）
- **比喩で直截な言明を置き換えない**：Anthropic が mannered prose と呼ぶ型の除去。比喩そのものは禁じず、直截な言い換えが存在する箇所に限った。無制限に適用すると、説明のための比喩まで消えるため
- **書き始める前に決めること**：読み手・アウトプット・課題の3点は、もとは formal だけの規定だったが、材料が増えて焦点が散る問題は casual でも起きる（I6）。3点を common に移し、formal には詰め方の観点と skills-forward の回避だけを残した
- **主張**：書き始める前に決める4点目として、著者の主張の有無と内容を加えた。主張のある文章では主張がもっとも大切で、根拠はそれを支えるために置く。較正の規定を著者の主張にまで当てると、主張が限定や TODO に埋もれるため、較正は事実の記述に限った。調査レポートやまとめのように主張の無い文章では、主張を作らない。体験の創作禁止と同じく、著者が示していない主張を AI が補うことを禁じた

追加にあたっては、規定そのものが埋め草を生まないかを点検した（I5）。書き始める前の3点は本文に節として書き出す必要が無いと明記した。チェックリストの「書き始める前に決めたか」を満たすためだけに、読み手や課題を述べる段落が増えるのを防ぐためである。調査段階の案にあった「何でも3つ挙げる癖」は、出典が無く機械でも検出できないため入れていない。

casual の「人間味のある表現」には、材料の無い箇所を埋める道具として使われやすい旨の注意を添えた。casual 固有の話なので `casual.md` に置いた。

### formal の範囲を文体に絞った（2026-09-19）

2026-09-08 の改稿で formal に入れた規定のうち、設計判断を記録する文書（Design Doc）でだけ要るものを削除した。

- 本体を15〜20分で読める分量に収め、詳細を Appendix に送る
- 選択肢を MECE に並べ、推奨案を示す
- 後戻りできる決定か（one-way / two-way door）を明記する
- 効果・規模・条件を数値で書く
- Design Doc・仕様書の frontmatter（`title`・`date`・`version`）と、それを起点にした鮮度・改訂管理
- 採用案の根拠と却下した代替案を記録する
- 未決事項を「要確認」「未定」で明示する

いずれも、書かれる文書と書かれない文書があり、文書の目的で決まる。文章一般の文体スキルとして毎回チェックさせると、該当しない文書にもそれを書き足す方向に働く（[AI臭い文章の問題と対策](../ai-prose.md) の不変条件 I5）。Design Doc の校正には必要な観点なので、必要になれば Design Doc 用の別スキルで扱う。文書種別ごとの骨子の表（決定事項と根拠、未決事項の欄を含む）は、種別に応じて適用されるため残した。相対時間の禁止と絶対日付の規定も、文書種別に依存しないため残した。

### スキルを簡潔にした（2026-09-19）

formal の文書では `common.md` と `formal.md` を合わせて約480行が読み込まれていた。スキル作成ガイドの「Claude がすでに知っていることは書かない」と、指示が増えると遵守が下がるという知見（「チェックリストを killer items に絞った」を参照）に従い、次の3種類を削除・縮小した（計画 F、13件）。

- Claude が既定で守る作法：接続詞、前の文からの展開、曖昧さの排除、図版・表
- 同じ規定を示す例の重複：能動態、箇条書きの書式、段落・箇条書き・文末（casual）の悪い例
- ツールや他の節と重複するもの：1文150字（textlint）、用語の一貫性（共通チェックリスト）、formal の禁止事項（9項目すべてが本文と重複）

formal の文書で読み込む量は約350行、casual は約345行になった。あわせて casual の「文末表現の多様化」を目的の1文だけにした。文末の一覧と例が、断言を弱める語尾（「〜と言える」「〜と考えられる」など）を多様化の手段として勧めており、I9 とぶつかったため（F14）。削除した原文は [writing-style から削除した規定](./writing-style-archive.md) に残した。規定を戻すとき、または Design Doc の校正など別のスキルで使うときはそこから取る。

評価（`evals/`）の5ケースは、削った規定（能動態、接続詞など）をほとんど測っていない。削減の前後で評価が劣化しないことは、スキルの主な効果（較正、振り分け）が保たれていることの確認にとどまる。

### casual の人間味の規定を著者の材料に限った（2026-09-19）

casual には、人間らしさの要素（感情表現、判断・感想、比喩、参考記事の評価、賞賛の姿勢）を「足すべきもの」として書いた規定があった。材料が無くても AI がそれを作り足すため、[AI臭い文章の問題と対策](../ai-prose.md) の3節で退けた選択肢2（穴を人間らしい断定で埋める）を誘う。これらを「著者の材料があるときの書き方」に改めた（I1・I5・I9）。

- 感情・評価、判断・感想、経験談は、著者が示したものを削らず、ぼかさずに書く。AI は作らない
- 比喩は、著者が使ったものを著者の言い回しとして残す。書き換えるかどうかは著者が決め、AI は候補を示すだけにする。「難儀する」は慣用句、「glue script」は用語で、比喩の例から外した
- 参考記事や他者の成果への評価・感想は、著者が読んで評価したものか、出典のあるコミュニティの評価に限る。「基本的に賞賛する姿勢」は、賞賛の定型句を足させるため削った
- 「断言調と説明調のバランス」は削除した。「〜と言える」「〜となる」を足して断言を弱める規定で、水増しと主張の埋没を招く

### チェックリストを killer items に絞った（2026-09-19）

チェックリストは common 13・casual 13・formal 22 の計48項目あり、ほぼすべてが本文の規定の繰り返しだった。指示が増えると遵守が一様に下がるという研究と指針（[IFScale](https://arxiv.org/abs/2507.11538)、[HumanLayer の CLAUDE.md の指針](https://www.humanlayer.dev/blog/writing-a-good-claude-md)、[Claude Code の CLAUDE.md の文書](https://code.claude.com/docs/en/memory)）と、「linter の仕事を LLM にさせない」「決定的な処理はスクリプトに任せる」（HumanLayer、[スキル作成ガイド](https://platform.claude.com/docs/en/agents-and-tools/agent-skills/best-practices)）に従い、次の3つに分けた。

- **チェックリスト**：このスキル固有で、飛ばすと害が大きく、見落とされやすい項目だけ（common 7、formal 1、casual 0）。書き始める前の確認、事実と出典の照合、リンク先の確認、主張を作らない・弱めない、表記ゆれ、機械検査の通過、formal の骨子と結論の位置
- **ツール**：`lint.sh` と textlint。箇条書きの行末の句読点は `lint.sh` に検出を足した（formal だけの規定なので WARN）
- **著者に示すもの**：WARN の箇所、比喩の候補、残した TODO、節の役割。中身の判断は AI に合否を決めさせない（[AI臭い文章の問題と対策](../ai-prose.md) の I3）

リンク切れの確認は、根拠の源泉にあたるため外さなかった。段落の文数など、Claude が既定で守るものや、ツールが無く害の小さいものは外した。
...
- **チェックリストの追加** → 共通なら common、種別固有なら該当 style の「固有チェックリスト」へ。ただし項目は、このスキル固有で、飛ばすと害が大きく、見落とされやすいものに限る（合わせて5〜9項目が目安）。機械で拾えるものは `lint.sh`・textlint に、中身の判断が要るものは SKILL.md の工程9（著者に示すもの）に置く。理由は「チェックリストを killer items に絞った」を参照。
- **`lint.sh` の検出パターンを変える** → 検体（型を埋め込んだ下書き）と `docs/ai-prose.md` に当てて、検出件数の変化を確かめる。誤検出と見逃しの釣り合いは著者が決める。
